...
import com.example.bookmarketfront.provider.OrderSqlProvider;
...
public interface IOrderDao extends BaseMapper<Order> {
    ...
    @Results({
            ...
            @Result(property = "address", column = "aid", one = @One(select = "com.example.bookmarketfront.dao.IAddressDao.searchById")),
            @Result(property = "books", column = "oid", many = @Many(select = "com.example.bookmarketfront.dao.IOrderBookDao.getOrderDetail"))
    })
